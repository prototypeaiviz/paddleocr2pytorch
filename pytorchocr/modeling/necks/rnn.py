...
* After the transformer is done and restored back to 2048 channels via conv3, 
the code does cat(h, conv3_out) — concatenating original + attended features into 4096 channels.
* The stop_gradient=True means gradients don't flow back through h a second time,
 avoiding double-counting during backprop.
  conv4 and conv1x1 then fuse and compress this combined tensor back down to 120.
* The result is a 40-token sequence where each token contains both:
 raw backbone detail AND
  global context from the transformer — handed to the CTC decoder to read the final text.
"""
"""
Why do we compress 
Attention cost scales with dim² per token pair
Why keep the guide?
The transformer only sees compressed 120-dim features. It may lose fine details the backbone captured.
Concatenating the original 2048-dim guide gives conv4 both: attended context + original fine detail.
"""
